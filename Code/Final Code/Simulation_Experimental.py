...
for Nodes in [20]:
    ...
    for t in tspan:
        ...
        for z in range(Nodes):
            ...
            Temps = Integration_step(Gstep, 3, mf, Temps, Tf_prev, del_t,
                                     del_z)
    ...
    plt.plot(tspan/60, np.array(Tf_ult) - 273.15,
...
error = [abs(np.interp(t, tspan, Tf_ult) - np.interp(t, np.array(Times), Tout))
         for t in tspan]
