...
for iterations in [1500,2000,3000,8000]:
    start = default_timer()
    del_z = Ltube/Nodes
    Temps = np.array([25,25,25,25,25])+273.15
...
#    plt.plot(tspan/60, np.array(Tab_ult)-273.15, label = 'Tab: '+str(Nodes))
    print(default_timer()-start, ' for ',iterations,' Iterations')
    plt.plot(tspan/60, np.array(Tf_ult)-273.15, label = 'Iterations: '+ str(iterations))
